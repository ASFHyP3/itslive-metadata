...
def count_s3_files(s3_path, extension='.nc', workers=8, format_type='text', cache_dir=None):
    ...
    cache_file = os.path.join(
        cache_dir, f'{bucket_name}_{prefix.replace("/", "_")}_{extension.replace(".", "_")}.cache'
    )
    ...
    def process_directory(dir_path):
        ...
        try:
            ...
            for page in paginator.paginate(Bucket=bucket_name, Prefix=dir_path):
                if 'Contents' in page:
                    file_count += sum(1 for obj in page['Contents'] if obj['Key'].endswith(extension))
    ...
    if to_process:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            future_to_dir = {executor.submit(process_directory, dir_path): dir_path for dir_path in to_process}
...
def main():
    ...
    parser.add_argument(
        '--format', '-f', choices=['text', 'csv', 'json'], default='text', help='Output format (default: text)'
    )
